...
pub fn delete_prediction_event(
    conn: &SqliteConnection,
    prediction_event_id: i32,
) -> Result<usize, diesel::result::Error> {
    diesel::delete(schema::prediction_events::table.find(prediction_event_id)).execute(conn)
}
...
pub fn get_prediction_event(
    ...
    schema::prediction_events::table
        .find(prediction_event_id)
        .load::<PredictionEvent>(conn)
}
